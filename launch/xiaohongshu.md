...
### 第 1 期 · VOLERA（`out/xhs-ep1-volera.mp4`）✓ 已发布，已过审（9/3：478 浏览 / 17 赞 / 15 收藏 / 2 转发）
...
### 第 2 期 · PURA（`out/xhs-ep2-pura.mp4`）✓ 已发布 9/2，已过审（9/3：146 浏览 / 6 赞 / 1 收藏）
- **标题：** 这行字是液态玻璃做的，还能搅动
...
### 第 3 期 · BOREAL（`out/xhs-ep3-boreal.mp4`）✓ 已发布 9/3（审核中，发光穹顶封面）
- **标题：** 往下滚，穿过一整片雾
- **正文（实发版）：** 滚轮就是镜头轨道：往下滚，镜头穿过雾中穹顶，落到粒子居民面前。
  雾色 == 背景色，几何体融进大气——这是获奖站最常用的一招。整页只有一个 HTML 文件，
  零构建。配方做成了开源 skill，把视频里那句 prompt 贴进 Claude Code，就能造一个
  自己的雾中世界。发布前它还会自己截图三个尺寸检查，不满意自己改。
  在线 demo：motion-pages.pages.dev
  GitHub：github.com/jiangyurong609/motion-pages（MIT 开源）
  这是「一句 prompt」系列第 3 期，关注看下一期：360° 穹顶画廊 🖼
- **话题：** 同上
- **封面：** 智能推荐里的发光穹顶帧（原定粒子居民特写帧太暗，小图不可读，弃用）
...
**✓ 链接评论已补（9/3，网页版扫码登录后发出）** — ep1 / ep2 / 图文预热 /
视频预热 四条笔记各一条（文案两个变体防重复判定）。置顶仍需手机 App。
9/3 发现：视频预热笔记已涨到 74 赞 / 90 收藏 / 9 评论，是目前最强一条；
图文笔记有条 momo 的评论（"没配图再怎么 skill 也没用"）待回复。
