...
### 第 3 期 · BOREAL（`out/xhs-ep3-boreal.mp4`）✓ 已发布 9/3，已过审（9/5：1 赞 / 3 收藏；发光穹顶封面）
- **标题：** 往下滚，穿过一整片雾
...
### 第 4 期 · ARCHIVE°（`out/xhs-ep4-dome.mp4`）✓ 已发布 9/4，已过审（9/5：173 浏览 / 4 赞 / 4 收藏；默认首帧封面——白底卡片网格）
- **标题：** 点谁，镜头就飞到谁面前
- **正文（实发版）：** 拖一下，环视一整面穹顶画廊；点一张卡，镜头直接飞到它面前。
  作品卡贴在穹顶内侧，点击即聚焦——获奖站作品集最爱的布局。整页只有一个 HTML 文件，
  零构建。配方做成了开源 skill，把视频里那句 prompt 贴进 Claude Code，就能给自己的
  作品集造一座穹顶画廊。发布前它还会自己截图三个尺寸检查，不满意自己改。
  在线 demo：motion-pages.pages.dev
  GitHub：github.com/jiangyurong609/motion-pages（MIT 开源）
  这是「一句 prompt」系列第 4 期，关注看下一期：弹簧物理海报墙 📌
- **话题：** 同上
- **录制注：** dome 片段的飞入镜头按挂钟时间对齐裁剪（record-xhs-clips.py 的
  sync 机制——录屏时 drag 步进会拖慢 ~2 倍，不能信名义时长）。

### 第 5 期 · PAPERWORKS（`out/xhs-ep5-paper.mp4`）✓ 已发布 9/5（审核中，默认首帧封面——白墙彩色海报倾斜中，够亮）
- **标题：** 甩一下，整墙海报跟着晃
- **正文（实发版）：** 随手一甩，整面海报墙像纸一样弯过去、再弹回来；甩得越快，弯得越狠。
  无限拖拽、带惯性，点开任意一张看大图。没用任何动画库——纯 DOM + CSS 3D，每张海报
  一根小弹簧。整页只有一个 HTML 文件，零构建。配方做成了开源 skill，把视频里那句 prompt
  贴进 Claude Code，就能给自己的作品做一面会晃的海报墙。发布前它还会自己截图三个尺寸检查，
  不满意自己改。
  在线 demo：motion-pages.pages.dev
  GitHub：github.com/jiangyurong609/motion-pages（MIT 开源）
  这是「一句 prompt」系列第 5 期，关注看下一期：动效语法课——缓动到底怎么选 📐
- **话题：** #vibecoding #AI编程 #前端 #CSS #独立开发
- **录制注：** paper 片段同样用 sync 机制，点开海报大图对齐到 t≈9.3s；
  快甩（0.8–1.0s）出弯曲、慢拉（2.2s）出回弹，两种速度都要有。

### 后续期数（素材待录）：tempo（动效语法课，
最适合涨粉的知识型内容）→ fernline / sona / sylva。跑完 9 期后做一期合集
（「9 个获奖级动效，每个一句 prompt」）。
...
**✓ 评论回复已发（9/3）** — 4 条：Lucas Cheung（求链接→补链接）、又耳元香、
90度羊（致谢+导流系列）、momo（图文笔记的批评→认同+转视频系列说明）。
**✓ ep3 / ep4 链接评论已发（9/5）**，网页版作者评论，两条文案轮换。ep5 过审后同样补一条；置顶仍需手机 App。
